package zzh.source.srceng.ui.widget.tablayout;
...
public class VerticalTabLayout extends ScrollView {
  ...
  @Dimension(unit = Dimension.DP)
  private static final int TAB_MIN_HEIGHT_MARGIN = 26;  //默认 56
  ...
  /**
   * Gravity used to fill the {@link VerticalTabLayout} as much as possible. This option only takes
   * effect when used with {@link #MODE_FIXED} on non-landscape screens less than 600dp wide.
   *
  ...
  public VerticalTabLayout(
    ...
    super.addView(
        slidingTabIndicator,
        0,
        new ScrollView.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
    ...
    final TypedArray ta =
        context.obtainStyledAttributes(tabTextAppearance, R.styleable.TextAppearance);
    try {
      tabTextSize = ta.getDimensionPixelSize(R.styleable.TextAppearance_android_textSize, 0);
      tabTextColors =
          MaterialResources.getColorStateList(
              context, ta, R.styleable.TextAppearance_android_textColor);
    } finally {
      ta.recycle();
    }
    ...
    unboundedRipple = a.getBoolean(R.styleable.TabLayout_tabUnboundedRipple, true);
    a.recycle();
   ...
   * intrinsic height (preferred). Otherwise, the indicator will not be shown unless gravity is set
   * to {@link #INDICATOR_GRAVITY_STRETCH}, in which case it will ignore indicator height and
  ...
  /**
   * Returns whether this {@link VerticalTabLayout} has an unbounded ripple effect, or if ripple is
   ...
   * bottom of the tab. If {@code tabIndicatorColor} is specified via the VerticalTabLayout's style
   * or via {@link #setSelectedTabIndicatorColor(int)} the selection indicator will be tinted that
   * color. Otherwise, it will use the colors specified in the drawable.
   *
   ...
   * default, this is a line along the bottom of the tab. If {@code tabIndicatorColor} is specified
   * via the VerticalTabLayout's style or via {@link #setSelectedTabIndicatorColor(int)} the
   * selection indicator will be tinted that color. Otherwise, it will use the colors specified in
   * the drawable.
   *
   ...
   * clicks. The tabs displayed in this layout will be populated from the ViewPager adapter's page
   * titles.
   *
  ...
  private TabView createTabView(@NonNull final Tab tab) {
    ...
    // 设置点击效果为 "?android:attr/selectableItemBackgroundBorderless"
  ...
  public final class TabView extends LinearLayout {
    ...
    @Nullable private View badgeAnchorView; // 将类型设为 View
    ...
    public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
      ...
      if (maxHeight > 0
          && (specHeightMode == MeasureSpec.UNSPECIFIED || specHeightSize > maxHeight)) {
  ...
  private class SlidingTabIndicator extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
      ...
      // GRAVITY_CENTER will make all tabs the same height as the largest tab, and center them in
      // the
      // SlidingTabIndicator's height (with a "gutter" of padding on either side). If the Tabs do
      // not
